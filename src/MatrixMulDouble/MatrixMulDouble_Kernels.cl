...
__kernel void mmmKernel_local2(__global double4 *matrixA,
                               __global double4 *matrixB,
                               __global double4* matrixC,
                               int widthA,
                               __local double4 *blockA,
                               __local double4 *blockB)

{
    ...
    for(int i = 0; i < (temp / get_local_size(0)); i++)
    {
        ...
        /* Each local thread will read a double4 */
        for(int j = 0; j < get_local_size(0) << 2; j=j+4)
        {
            ...
            double4 tempB0 = blockB[get_local_id(0)  + j *  get_local_size(0)]; //Should be localId.x * (TILEX / 4)
            double4 tempB1 = blockB[get_local_id(0)  + (j + 1) *  get_local_size(0)];
            double4 tempB2 = blockB[get_local_id(0)  + (j + 2) *  get_local_size(0)];
            double4 tempB3 = blockB[get_local_id(0)  + (j + 3) *  get_local_size(0)];
